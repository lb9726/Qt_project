...
{
    if(!CopyFile(srcFile, dstFile, pControl, pShowFunc))
        return false;
    return DelFile(srcFile, pControl);
}

bool CopyFile(QString srcFile, QString dstFile, int *pControl, pShowInfo *pShowFunc)
{
    //>@如果路径相同，则不进行拷贝
#ifdef WINDOWS
...
bool CopyFolder(QString pSrcDir, QString pDstDir, bool pSubfolder, int *pControl, pShowInfo *pShowFunc)
{
    ...
    foreach(QFileInfo tmpFileInfo, tmpFileList)
    {
        ...
        CopyFile(tmpSrcFilePath, tmpDstFilePath, pControl, pShowFunc);
        if(pShowFunc)
        {
...
void FileOperate::CopyFile(QString srcFile, QString dstFile, pShowInfo *pShowFunc)
{
    m_Src = srcFile;
    m_Dst = dstFile;
    m_ShowFunc = pShowFunc;
...
void FileOperate::run()
{
    ...
    switch(m_State)
    {
        ...
        case S_FILE_COPY:   //>@CopyFile
        {
            tmpRet = ::CopyFile(m_Src, m_Dst, &m_StopFlag, m_ShowFunc);
            break;
        }
