...
{
    if(!pFile.startsWith(pDir, Qt::CaseInsensitive))  //>@主题包内资源
        return;
...
UiRcParser* UiRcListParser::NextRc()
{
    ...
    if(tmpKeyList.count() > 0)
    {
        ...
        if(index < 0)
        {
            mCurKey = tmpKeyList.last();  //>@如果找不到，就返回最后一个
        }
        else
        {
...
UiRcParser* UiRcListParser::PrevRc()
{
    ...
    if(tmpKeyList.count() > 0)
    {
        ...
        if(index < 0)
        {
            mCurKey = tmpKeyList.first();    //>@如果找不到，就返回第一个
        }
        else
        {
            if((index-1) >= 0)
...
void UiOperateParser::slot_Finished()  //暂无用处
{
    IDE_TRACE();
