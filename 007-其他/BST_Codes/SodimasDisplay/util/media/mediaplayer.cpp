...
bool MediaPlayer::findInMediaBuffer(QString pAudioFile, QList<PLAY_MEDIA_INFO*> &pResultList, bool pFindall)
{
    pResultList.clear();
...
bool MediaPlayer::testMedia(int pAoType, QString pSrc, int pVolume)
{
...
void MediaPlayer::setMixerDriver(QString pDriver)
{
    m_MixerDriver = pDriver;
}

///设置音量、驱动
void MediaPlayer::setAoDriver(QString pDriver)
{
    m_AoDriver = pDriver;
}
...
void MediaPlayer::abort()  ///只在线程中使用
{
    m_AbortAudio = true;
}
...
void MediaPlayer::audioFinished()
{
    m_AoMode = AO_IDEL;
    m_AbortAudio = false;
    m_Pause = false;
