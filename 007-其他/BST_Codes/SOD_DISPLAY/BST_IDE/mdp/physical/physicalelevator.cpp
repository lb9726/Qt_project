...
void PhysicalElevator::GotoTest(bool pEnable, bool pNormal)
{
    ...
    else
    {
        ...
        if(mAutoTestEnable != pEnable)
            emit sInfo(QString("Autotest finished!"), 1000);
        if(pNormal)
        {
...
bool PhysicalCOM::SetValue(QString pValue)
{
    ...
    {
        //        if(!m_LCD.isNull())  //jie jue duoci ba cha CAN shi de standby mo shi.
        //        {
        ...
        //            }
        //        }
//        Disable by lisx 20170223
//        if(!mPlayer.isNull())
//        {
//          if(mPlayer.data()->mPlayerList[0])
//              mPlayer.data()->mPlayerList[0]->Stop(true, 2);
//          if(mPlayer.data()->mPlayerList[1])
//              mPlayer.data()->mPlayerList[1]->Stop(true, 2);
//        }
    }
        ...
        if(!pFrame->mData.isEmpty())
        {
            ...
            foreach(tmpPara, tmpParaList)
            {
                ...
                if(!tmpSubParaList.at(0).compare("APP", Qt::CaseInsensitive))
                {
                    ...
                    emit sInfo(QString("I:%1_C:%2").arg(mInterBoardIAPVersion).arg(mInterBoardVersion), MSG_PERMANENT, DM_NORMAL);
                }
                else if(!tmpSubParaList.at(0).compare("IAP", Qt::CaseInsensitive))
                {
                    mInterBoardIAPVersion = tmpSubParaList.at(1);
