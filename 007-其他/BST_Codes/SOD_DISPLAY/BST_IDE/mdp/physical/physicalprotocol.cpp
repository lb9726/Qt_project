...
quint32 PhysicalProtocol::ParseFrameBuffer(PH_Type pType, QIODevice *pDev, QByteArray pFrameBuffer)
{
    ...
    //if(mDevType == P_TCP||mDevType == P_UDP){
    //    L_IDE_DEBUG(QString("Get Protocol -- dev %1 [%2 bytes]").arg(mDevType).arg(tmpCount));
    //    for(int m=0;m<pFrameBuffer.count();m++)
    //    {
    //        L_IDE_TRACE_INT((quint8)(pFrameBuffer.at(m)));
    //    }
    //}
...
quint32 PhysicalProtocol::ParseConfFrame(FRAME_STRUCT *pFrame)
{
    ...
    L_IDE_DEBUG(QString("Parse Conf -- dev %1 [%2]").arg(pFrame->mDevType).arg(tmpRecv));
...
quint32 PhysicalProtocol::ParseFileFrame(FRAME_STRUCT *pFrame, bool pEmitPrg)
{
    ...
    {
        PrintFrame("ParseFileFrame from APP",pFrame);
        ...
        case Flag_FileFooter:
        {
            ...
            PrintFrame("ParseFileFrame ACK to APP's ",pFrame);
    ...
    {
        PrintFrame("ParseFileFrame APP's ACK",pFrame);
        bool tmpStop = pFrame->mCmd & Flag_Stop;
        if(tmpStop)
        {
...
{
    PrintFrame("ParseHeartbeatFrame",pFrame);
...
quint32 PhysicalProtocol::ReplyHBFrame(FRAME_STRUCT *pFrame, QByteArray pContent)
{
    ...
    PrintFrame("ReplyHBFrame",pFrame);
    return ReplyFrame(pFrame);
}


void PhysicalProtocol::PrintFrame(QString name,FRAME_STRUCT *pFrame){
    L_IDE_DEBUG(QString("+++%1").arg(name));

    int len = pFrame->mData.count();
    for(int m=0;m<len;m++)
    {
        L_IDE_TRACE_INT((quint8)(pFrame->mData.at(m)));
    }
...
quint32 PhysicalProtocol::SendHBFrame(quint32 pTimeout, bool pAck)
{
    ...
    PrintFrame("SendHBFrame",tmpFrame);
    return SendFrame(tmpFrame);
}
...
quint32 PhysicalProtocol::ReplyConfFrame(SetCmdInfo pInfo, FRAME_STRUCT* pFrame)
{
    ...
    L_IDE_DEBUG(QString("Reply Conf -- dev %1 [%2]").arg(pFrame->mDevType).arg(tmpInstruc));
...
quint32 PhysicalProtocol::SendFileData(FRAME_STRUCT* pPreFrame, bool pEmitPrg)
{
    ...
    PrintFrame("SendFileData to APP ",tmpFrame);
...
quint32 PhysicalProtocol::SendFileMD5(FRAME_STRUCT* pPreFrame)
{
    ...
    PrintFrame("SendFileMD5 to APP ",tmpFrame);
    return SendFrame(tmpFrame);
}
