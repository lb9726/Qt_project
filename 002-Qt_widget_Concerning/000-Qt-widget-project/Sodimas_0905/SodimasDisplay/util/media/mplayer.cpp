...
/*
int mainPlayer_fd;
int mianPlayerPipe[2];

QString mianMusicPath = "/opt/M1.wav";
bool initMainplayerProcess(){
    pid_t pid;
    unlink("/tmp/mainplayer_fifo");
    if(mkfifo("/tmp/mainplayer_fifo",O_CREAT|0666) == -1){
        return false;
    }
    if(pipe(mianPlayerPipe) ==-1){
        return false;
    }
    if((pid=fork()) == -1){
        return false;
    }else if(pid==0){
        close(mianPlayerPipe[0]);
        dup2(mianPlayerPipe[1],1);//"−codecs-file","/etc/asound.conf",
        ///execlp("mplayer","mplayer","-slave","-quiet","-input","file=/tmp/mainplayer_fifo","-loop","0","-ao","alsa","-volume","30",mianMusicPath.toStdString().c_str(),NULL);
        execlp("mplayer","mplayer","-slave","-quiet","-input","file=/tmp/mainplayer_fifo","-ao","alsa","-loop","0","-volume","30",mianMusicPath.toStdString().c_str(),NULL);
    }else{
        if((mainPlayer_fd = open("/tmp/mainplayer_fifo",O_RDWR))==-1)
        {
            IDE_TRACE();
            return false;
        }
        write(mainPlayer_fd,PAUSE_PLAYER,strlen(PAUSE_PLAYER));
    }
    return true;
}
*/
...
bool initBackgroundplayerProcess(){
    ...
    if((pid=fork()) == -1){
        ...
        execlp("mplayer","mplayer","-slave","-quiet","-input","file=/tmp/musicplayer_fifo",\
               ...
               backgroundMusicPath.toStdString().c_str(),NULL);
    }else{
