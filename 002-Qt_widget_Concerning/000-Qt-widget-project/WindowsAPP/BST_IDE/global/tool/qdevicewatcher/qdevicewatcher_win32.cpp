...
LRESULT CALLBACK dw_internal_proc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    {
        DEV_BROADCAST_HDR *lpdb = (DEV_BROADCAST_HDR *)lParam;  //>@判断类型
        ...
        {
            if (lpdb->dbch_devicetype == DBT_DEVTYP_OEM)
            {
                zDebug("DBT_DEVTYP_OEM");
                DEV_BROADCAST_OEM *pDevOem = (DEV_BROADCAST_OEM*)lpdb;
            }
            else if (lpdb->dbch_devicetype == DBT_DEVTYP_DEVNODE)
            {
                zDebug("DBT_DEVTYP_DEVNODE");
            }
            else if (lpdb->dbch_devicetype == DBT_DEVTYP_VOLUME)
            {
                ...
                foreach (const QString &drive, drives)
                {
                    ...
                    watcher->emitDeviceAction(QString("storage>")+drive, action_str);
            ...
            else if (lpdb->dbch_devicetype == DBT_DEVTYP_NET)
            {
                zDebug("DBT_DEVTYP_NET");
            }
