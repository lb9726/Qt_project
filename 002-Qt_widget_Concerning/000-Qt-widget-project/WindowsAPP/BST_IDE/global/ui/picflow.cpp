...
PictureFlow::PictureFlow(QWidget* parent):
...
PicFlowBrowser::PicFlowBrowser():
    PictureFlow()
{
    setWindowTitle("PictureFlow");

    QVBoxLayout *tmpLayout = new QVBoxLayout;
    QHBoxLayout *tmpBtnLayout = new QHBoxLayout;
    QPushButton *tmpBtn = new QPushButton(this);
    tmpBtn->setFixedSize(160, 50);
    tmpBtn->setText("EXIT");
    tmpBtn->setFlat(true);
    tmpBtn->setStyleSheet("font: bold large \"Adobe Arabic\"; font-size:30pt; color: rgba(255,10,20,250);");
    tmpBtnLayout->addSpacerItem(new QSpacerItem(40, 20, QSizePolicy::Expanding, QSizePolicy::Minimum));
    tmpBtnLayout->addWidget(tmpBtn);
    tmpLayout->addLayout(tmpBtnLayout);
    tmpLayout->addSpacerItem(new QSpacerItem(20, 40, QSizePolicy::Minimum, QSizePolicy::Expanding));
    setLayout(tmpLayout);

    connect(tmpBtn, SIGNAL(clicked()), this, SLOT(slot_Close()));
}

void PicFlowBrowser::slot_Close()
{
    QDialog::accept();
}

void PicFlowBrowser::keyPressEvent(QKeyEvent* event)
{
    ...
    if(event->key() == Qt::Key_F10)
    {
        if(event->modifiers() == Qt::AltModifier)
        {
            ...
            {
                render(); repaint(); QApplication::flush();
#ifndef QT5
                QApplication::syncX();
#endif
                render(); repaint(); QApplication::flush();
