...
#define BVT_ESC_AF  0xFE     /* 转换字符转换后增加字符 不能写成0x00,字符串遇到0x00,就直接结束了*/
...
void WifiCommunication::parserJsonFormat(const QByteArray &byteArray)
{
    ...
    else if (!QString::compare(type, "webUpload", Qt::CaseInsensitive)) // webUpload
    {
        webUpload(jsonInfo);
    }
    else if (!QString::compare(type, "webUploadShowMessage", Qt::CaseInsensitive)) // webUploadShowMessage
    {
        webShowUploadFileMessage(jsonInfo);
    }
    else
    {
...
void WifiCommunication::webUpload(QJsonValue &InfoMap)
{
    //    qDebug()<<__PRETTY_FUNCTION__<<"lines = "<<__LINE__<<"InfoMap = "<<InfoMap;
        if (QJsonValue::Object == InfoMap.type())
        {
            QJsonObject webUploadFileObj = InfoMap.toObject();

            QJsonValue webUploadFileFlag = webUploadFileObj["flag"];
            bool tmpFlag = false;
            if (QJsonValue::Bool == webUploadFileFlag.type())
            {
                tmpFlag = webUploadFileFlag.toBool();
            }
            else
            {
                qDebug()<<__PRETTY_FUNCTION__<<"lines = "<<__LINE__<<"is not Bool";
            ...
            QString fileNameString;
            QJsonValue tmpfileName = webUploadFileObj["filename"];
            if (QJsonValue::String == tmpfileName.type())
            {
                fileNameString = tmpfileName.toString();
            }
            ...
            if (tmpFlag)
            {
                emit sigWebUpload(fileNameString);
            }
            qDebug()<<__PRETTY_FUNCTION__<<"lines = "<<__LINE__<<tmpFlag<<fileNameString;
        }
        else
...
void WifiCommunication::webShowUploadFileMessage(QJsonValue &InfoMap)
{
    if (QJsonValue::Bool == InfoMap.type())
    {
        bool tmpFlag = InfoMap.toBool();
        emit sigShowUploadFileMessage(tmpFlag);
    }
    else
    {
        qDebug()<<__PRETTY_FUNCTION__<<"lines = "<<__LINE__<<"is not Bool";
