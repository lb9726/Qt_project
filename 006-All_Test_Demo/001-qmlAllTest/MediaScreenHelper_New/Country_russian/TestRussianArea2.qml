...
Item {
    ...
    function area2_parameter_screenSaver()
    {
        if (isStandby)
        {
            ConfigureSerialer.updateStandby(vStage1Interval,
                vStage1Brightness, vStage2Interval, vStage2Brightness);
        }
    }
    ...
    function updateParameterSetting2() {
        ...
        if ("false" === res)
        {
            ...
            if (!isDateTime && !isStandby)
            {
                ...
                ConfigureSerialer.updateParameterBasic2(false,
                    !isDateTime,  isStandby, !isDateTime, isAudioSel, isAudioOff,
                    hasChoiceTimeAreaFlag, hasChoiceAudioSwitchFlag)
            }
            else
            {
                console.log("false isDateTime = ", isDateTime, isStandby)
                ConfigureSerialer.updateParameterBasic2(true, !isDateTime,  isStandby,
                !isDateTime, isAudioSel, isAudioOff, hasChoiceTimeAreaFlag, hasChoiceAudioSwitchFlag)
            }
        }
        else
        {
            ConfigureSerialer.updateParameterBasic2(true,
                !isDateTime,  isStandby, !isDateTime, isAudioSel, isAudioOff, hasChoiceTimeAreaFlag, hasChoiceAudioSwitchFlag)
        }
    ...
    function getIpInformation()
    {
        ...
        ConfigureSerialer.setIpParameter(autoSelect.checked,
            pIpAddress, pSubnetMask, pDefaultGateWay, pDnsService)
    }
}
