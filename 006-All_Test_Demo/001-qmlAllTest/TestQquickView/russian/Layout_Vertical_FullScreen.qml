...
//        }
        ...
        MediaPlayer{
            ...
            onStopped: {
                console.log("layout_vertical_fullscreen in russian the media is end!")
            }
            onPlaying: {
                console.log("layout_vertical_fullscreen in russian is playing")
            }
        }
        VideoOutput {
            width: parent.width; height: parent.height
        ...
        MouseArea {
            anchors.fill: parent
            onClicked: {
                pauseorplay(); console.log("videopath in russian = ", videoPath);
            }
        }
    }
    Item {
    ...
    {
        videoPath = svp;
        console.log("svp videopath verticalfullscreen in russian = ", svp)
    }
...
//        }
    ...
    function stopVideo()
    {
        ...
        cycleImage.stop();
        anima_scroll.stop();
        console.log("in russian vertical_fullscreen stopvideo")
    }
    ...
    function stopAudio()
    {
        if (isAudio)
        {
            ...
            {
                player.stop();
                console.log("receive signal stop audio in vertical fullscreen in russian")
            }
        }
        else if (isVideo)
        {
            ...
            {
                player.stop();
                console.log("receive signal stop video in vertical fullscreen in russian")
            }
        }
    }
    ...
    {
        console.log("in russian Vertical playStatus = ", playStatus);
        if (isVideo)
        {
            if (0 === playStatus)
            {
        ...
        else if (isAudio)
        {
            if (1 === playStatus)
            {
                playStatus = 0;
                player.pause();
                console.log("Vertical fullscreen in russian is call pause");
            }
            else
            {
                playStatus = 1;
                player.play();
                console.log("Vertical fullscreen in russian is call play");
            }
        }
    }
    ...
    function autoPlay_Audio()
    {
        if (isAudio)
        {
            if (player.PlayingState !== player.playbackState)
                player.play();
            console.log("functiong in autoplay in layout_vertical_fullscreen in russian languange")
        }
        else if (isVideo)
        {
            if (player.PlayingState !== player.playbackState)
                player.play();
            console.log("video functiong in autoplay in layout_vertical_fullscreen in russian languange")
        }
    }
    ...
    function clearAllParament()
    {
        ...
        isPicture  = false;
        videoPath = "";
        console.log("vertical fullscreen in russian is clearpara")
    }
    ...
    Connections {
        ...
        onClearAudioParament: {
            if (3 != lan_index)
            {
                console.log("vertical fullscreen in russian lan_index = ", lan_index);
                clearAllParament();
            }
        }
    }
    ...
    Connections {
        ...
        onVerticalFullscreenAudioAutoPlay: {
            autoPlay_Audio();
            console.log("vertical_fullscreen in russian audio will play")
        }
    }
    ...
    Connections {
        ...
        onStopAllAudioPlay: {
            stopAudio();
            console.log("stopAudio is call in vertical fullscreen in russian")
        }
    }

    Component.onCompleted: {
        console.log("layout_vertical_fullscreen is completed in russian")
    }
}
