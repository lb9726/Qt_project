...
//        }
        ...
        MediaPlayer {
            ...
            onPlaying: {
                console.log("layout_horizontal_fullscreen is playing")
            }
        }
        VideoOutput {
            width: parent.width; height: parent.height
    ...
    {
        videoPath = svp;
        console.log("svp videopath horizontalfullscreen = ", svp)
    }
...
//        anima_scroll.stop();
    ...
    function stopAudio()
    {
        if (isAudio)
        {
            ...
            {
                player.stop();
                console.log("receive signal stop audio in horizontal fullscreen")
            }
        }
    }
    ...
    function pauseorplay()
    {
        ...
        else if (isAudio)
        {
            if (1 === playStatus)
            {
                playStatus = 0;
                player.pause();
                console.log("Horizontal fullscreen is call pause");
            }
            else
            {
                playStatus = 1;
                player.play();
                console.log("Horizontal fullscreen is call play");
            }
        }
    }
    ...
    function autoPlay_Audio()
    {
        if (isAudio)
        {
            if (player.PlayingState !== player.playbackState)
                player.play();
            console.log("functiong in autoplay in layout_horizontal_fullscreen in normal languange")
        }
        else if (isVideo)
        {
            if (player.PlayingState !== player.playbackState)
                player.play();
            console.log("video functiong in autoplay in layout_horizontal_fullscreen in normal languange")
        }
    }
    ...
    function clearAllParament()
    {
        ...
        isPicture  = false;
        videoPath = "";
        console.log("horizontal fullscreen in normal is clearpara")
    }
    ...
    Connections {
        ...
        onClearAudioParament: {
            if (3 == lan_index)
            {
                console.log("horizontal fullscreen in normal is clearPara lan_index = ", lan_index);
                clearAllParament();
            }
        }
    }
    ...
    Connections {
        ...
        onHorizontalFullscreenAudioAutoPlay: {
            autoPlay_Audio();
            console.log("horizontal_fullscreen audio will play")
        }
    }
    ...
    Connections {
        ...
        onStopAllAudioPlay: {
            stopAudio();
            console.log("stopAudio is call in horizontal fullscreen")
        }
    }
    ...
    Connections {
        ...
        onStartPlayAfterCloseFullScreen: {
            autoPlay_Audio();
            console.log("function in close horizontal fullscreen");
        }
    }

    Component.onCompleted: {
        console.log("layout_horizontal fullscreen is completed")
    }
}
