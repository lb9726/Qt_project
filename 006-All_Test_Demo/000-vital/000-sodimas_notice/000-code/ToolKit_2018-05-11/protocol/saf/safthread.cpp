...
void SafThread::run()
{
    qDebug()<<__PRETTY_FUNCTION__<<"is runing";
   while(1)
   {
       if(s_gpioChangeFlag)
       {
...
//    以上特殊功能只有图标，没有音频，因此插入后，重复发楼层帧不会重复播报
    ...
    {
        //qDebug()<<"saf zai zhe li you da yin me ?";
...
void SafThread::parserGpioNum()
{
    s_LastSaf = s_saf; // record last value
    s_saf = gpioValues[4];
    if(s_saf >= 70)
    {
        s_saf = s_LastSaf;
