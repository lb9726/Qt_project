...
//    connect(m_saf, SIGNAL(sigFloorArrive(int, int, int, int)), this, SLOT(slot_FloorArrive(int, int, int, int)));
...
//            disconnect(m_saf, SIGNAL(sigFloorArrive(int, int, int, int)), this, SLOT(slot_FloorArrive(int, int, int, int)));
...
void Uiloader::sltDeviceDisConnected()
{
    if (mLabelWIFI)
    {
        ...
        if (m_canopen)
        {
            QString tmp = mCanopenMessageStatus + " device is disconnected\n";
            if (mIsSendZipFile && mIsSendZipFileProgress > 0 && mIsSendZipFileProgress < 100)
            {
                tmp = tmp + " sendFile is interrupted!\n";
                if (mZipConcernProgress)
                {
                    mZipConcernProgress->hide();
                }
...
//            }
        ...
        {
            QString tmp = mStatusSafProtocol + " device is disconnected\n";
            if (mIsSendZipFile && mIsSendZipFileProgress > 0 && mIsSendZipFileProgress < 100)
            {
                tmp = tmp + " sendFile is interrupted!\n";
                if (mZipConcernProgress)
...
//            QString tmp = mFtpSendFileName.split("/").last();
//            qDebug()<<__PRETTY_FUNCTION__<<"lines = "<<__LINE__<<"tmp = "<<tmp;
//            mZipFunction->Unzip(tmp);
        }
//        mZipFunction.startUnZip(mFtpSendFileName, "/usr/bst", true, true);
    ...
    else if(!QString::compare(key, "autotest"))
    {
        ...
        if (m_autotest != pAutoTest)
        {
            ...
            if (m_autotest)
            {
                ...
                {
                    disconnectSafSignal(m_autotest);
                }
                slot_AutoTest();
                QtSleep(10);
                mAutoTestTimer.setInterval(m_testfreq);
...
//        mZipFunction.startZip("/usr/bst/usrfs/", ftpFilePath + fileName, true, true);
...
//        connect(mZipFunction, &ZipAndUnZip::zipProgress, this, &Uiloader::sltZipProgress, Qt::QueuedConnection);
//        connect(mZipFunction, &ZipAndUnZip::unZipProgress, this, &Uiloader::sltUnZipProgress, Qt::QueuedConnection);
//        connect(mZipFunction, &ZipAndUnZip::end, this, &Uiloader::sltZipOrUnZipEnded, Qt::QueuedConnection);
//        connect(mZipFunction, &ZipAndUnZip::error, this, &Uiloader::sltUnzipFileErrors, Qt::QueuedConnection);
//    //    connect(&mZipFunction, &ZipFunction::message, this, &Uiloader::sltShowZipConcernMessage, Qt::QueuedConnection);
//        connect(mZipFunction, &ZipAndUnZip::workStart, this, &Uiloader::sltShowZipConcernMessage, Qt::QueuedConnection);
    }
}
...
void Uiloader::sltUnZipProgress(quint32 pro)
{
    ...
    if (100 == pro)
    {
        if (m_messageBar->m_messageLabel)
        {
            QString tmp = " unZip " + mFtpSendFileName.split("/").last() + "Successed!\n";
            m_messageBar->m_messageLabel->setText(tmp);
        }
        num = 0;
    }
...
void Uiloader::sltUploadProgress(int progress)
{
    ...
    if (m_messageBar)
    {
        ...
        else
        {
            if (m_messageBar->m_messageLabel)
            {
                QString tmp = " Transfer " + mFtpSendFileName.split("/").last() + " Successed!\n";
                m_messageBar->m_messageLabel->setText(tmp);
                m_messageBar->show();
            }
        }
