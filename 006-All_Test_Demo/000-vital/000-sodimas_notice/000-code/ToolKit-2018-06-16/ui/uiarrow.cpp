...
void UiArrow::StartWork(int pScrollLevel)
{
    if(pScrollLevel != 0)
    {
        ...
        {
            qDebug()<<__PRETTY_FUNCTION__<<"lines = "<<__LINE__<<"is not running start";
            timeline->start();
        }
        else
        {
            qDebug()<<__PRETTY_FUNCTION__<<"lines = "<<__LINE__<<"already running Escape";
        }
    }
}
