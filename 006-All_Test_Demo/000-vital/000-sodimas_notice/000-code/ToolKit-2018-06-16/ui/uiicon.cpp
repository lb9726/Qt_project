...
void UiIcon::Init(QDomElement pElement)
{
    ...
    if (mTextLine1)
    {
        tmpRect.setHeight(tmpRect.height() + mTextLine1->height());
    }
...
void UiIcon::updateIcon()
{
    ...
    if (tmpStackedWidget && tmpStackedWidget->currentIndex() == 0)
    {
        ...
        if (!tmpElement.isNull())
        {
            ...
            QStringList tmpList = tmpText.split(";", QString::SkipEmptyParts);
            if (tmpList.count() == 1)
            {
                mTextLine1->clear();
                mTextLine2->clear();
...
void UiIcon::setEmptyIcon()
{
    qDebug()<<__PRETTY_FUNCTION__<<"lines = "<<__LINE__<<"clear Icon";
    mIcon->setPixmap(QPixmap());
}

void UiIcon::setBigTextEmpty()
{
    if (mBigTextLine1)
    {
        mBigTextLine1->clear();
...
void UiIcon::setLabelStyleSheet()
{
    ...
    if (mBigTextLine1)
    {
        ...
        mBigTextLine1->setAlignment(Qt::AlignLeft);
    }
    if (mBigTextLine2)
    {
        mBigTextLine2->setFont(font);
        mBigTextLine2->setAlignment(Qt::AlignLeft);
    }
    if (mBigTextLine3)
    {
        mBigTextLine3->setFont(font);
        mBigTextLine3->setAlignment(Qt::AlignLeft);
    }
    if (mBigTextLine4)
    {
        mBigTextLine4->setFont(font);
        mBigTextLine4->setAlignment(Qt::AlignLeft);
    }
    if (mTextView)
    {
...
void UiIcon::setLabelGeometry()
{
    if(gUiLoader->m_themeDirection == Horizontal)
    {
        if (mBigTextLine1)
        {
            mBigTextLine1->setGeometry(120, 56, 240, 40);
        }
        if (mBigTextLine2)
        {
            mBigTextLine2->setGeometry(120, 96, 240, 40);
        }
        if (mBigTextLine3)
        {
            mBigTextLine3->setGeometry(120, 136, 240, 40);
        }
        if (mBigTextLine4)
        {
            mBigTextLine4->setGeometry(120, 176, 240, 40);
        }
        if (mUsbPicture)
        {
        ...
        if (mTextView)
        {
            mTextView->setGeometry(90, 30, 301, 178);
        }
        if (mCopyProgress)
        {
    ...
    else
    {
        if (mBigTextLine1)
        {
            mBigTextLine1->setGeometry(35, 180, 240, 40);
        }
        if (mBigTextLine2)
        {
            mBigTextLine2->setGeometry(35, 220, 240, 40);
        }
        if (mBigTextLine3)
        {
            mBigTextLine3->setGeometry(35, 260, 240, 40);
        }
        if (mBigTextLine4)
        {
            mBigTextLine4->setGeometry(35, 300, 240, 40);
        }
        if (mUsbPicture)
        {
...
void UiIcon::showBackUpScreen()
{
    ...
    if (mUsbAct.dirIsEmpty("/media/sodimas/usrfs/"))
    {
        setUsbUI(false, "U disk has no resource!");
        return;
    }
