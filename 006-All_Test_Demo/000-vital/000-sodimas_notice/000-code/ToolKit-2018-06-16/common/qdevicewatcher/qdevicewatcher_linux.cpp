...
#define USE_REGEXP 0
    ...
    static bool sUsbDev = false;
    QString action_str, devinfo, dev;

    if(sUsbDev)
    {
        int index = line.indexOf("POWER_SUPPLY_PRESENT");
        if(index < 0)
            return;
        QString tmpNum = line.mid(strlen("POWER_SUPPLY_PRESENT")+1, 1);
        int tmpValue = tmpNum.toInt();
        //zDebug("++++++++++++++++++++++++++++%d", tmpValue);
        if(tmpValue == 1)
            action_str = "add";
        else
    ...
    else
    {
        ...
        //zDebug("+++++++++++++++++++++++++++123");
        if (line.contains("/block/")) //Storage
        {
            action_str = line.left(index).toLower();
            dev = "/dev/" + line.right(line.length() - line.lastIndexOf('/') - 1);
            devinfo = QString("storage>") + dev;
        }
        else if (line.contains("/event")) //touch
        {
            action_str = line.left(index).toLower();
            dev = "/dev/" + line.right(line.length() - line.lastIndexOf('/') - 1);
            devinfo = QString("touch>") + dev;
        }
        else if (line.contains("/power_supply/")) //usb device
        {
            //action_str = line.left(index).toLower();
            //dev = "/dev/" + line.right(line.length() - line.lastIndexOf('/') - 1);
            //devinfo = QString("usbdev>") + dev;
        ...
        else if (line.contains("/video4linux/")) //Camera
        {
            action_str = line.left(index).toLower();
            dev = "/dev/" + line.right(line.length() - line.lastIndexOf('/') - 1);
            devinfo = QString("camera>") + dev;
        }
        else if (line.contains("/sound/card")) //Audio
        {
            action_str = line.left(index).toLower();
            dev = "/dev/snd/" + line.right(line.length() - line.lastIndexOf('/') - 1);
            devinfo = QString("sound>") + dev;
        }
        else if (line.contains("/net/")) //wifi
        {
            action_str = line.left(index).toLower();
            dev = line.right(line.length() - line.lastIndexOf('/') - 1);
            devinfo = QString("net>") + dev;
        }
        else
        {
            return;
    ...
    {
        emitDeviceAdded(devinfo);
        event = new QDeviceChangeEvent(QDeviceChangeEvent::Add, devinfo);
    }
    else if (action_str==QLatin1String("remove"))
    {
        emitDeviceRemoved(devinfo);
        event = new QDeviceChangeEvent(QDeviceChangeEvent::Remove, devinfo);
    }
    else if (action_str==QLatin1String("change"))
    {
        emitDeviceChanged(devinfo);
        event = new QDeviceChangeEvent(QDeviceChangeEvent::Change, devinfo);
	}
