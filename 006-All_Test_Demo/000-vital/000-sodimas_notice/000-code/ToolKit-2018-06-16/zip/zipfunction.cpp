...
{
    qDebug()<<__PRETTY_FUNCTION__<<"is enter run";
    if (m_ZipFlag)
    {
    ...
    {
        qDebug()<<__PRETTY_FUNCTION__<<"m_UnZipFlag = "<<m_UnZipFlag;
...
//        qDebug()<<__PRETTY_FUNCTION__<<"is file";
        ...
        {
            qDebug()<<__PRETTY_FUNCTION__<<"lines = "<<__LINE__<<"is ZipFile";
            emit error();
            QCoreApplication::processEvents(QEventLoop::AllEvents, 1);
            return false;
        }
//    qDebug()<<__PRETTY_FUNCTION__<<"start zip";
        ...
        {
            qDebug()<<__PRETTY_FUNCTION__<<"lines = "<<__LINE__<<" inFile open error";
            zip.close();
            emit error();
            QCoreApplication::processEvents(QEventLoop::AllEvents, 1);
            return false;
        ...
        {
            qDebug()<<__PRETTY_FUNCTION__<<"lines = "<<__LINE__<<" outFile open error";
            inFile.close();
            zip.close();
            emit error();
            QCoreApplication::processEvents(QEventLoop::AllEvents, 1);
...
//            emit message(QString("Zip %1 to %2 error!").arg(pSrcPath).arg(pZipFile));
        ...
        {
            qDebug()<<__PRETTY_FUNCTION__<<"lines = "<<__LINE__<<" zip.getZipError() != UNZ_OK";
//            emit message(QString("Zip %1 to %2 error!").arg(pSrcPath).arg(pZipFile));
            emit error();
            QCoreApplication::processEvents(QEventLoop::AllEvents, 1);
            return false;
    ...
    else if(tmpInfo.isDir())
    {
        if(!tmpInfo.exists())
        {
            qDebug()<<__PRETTY_FUNCTION__<<"lines = "<<__LINE__<<pSrcPath<< " is NotExist";
            emit error();
            QCoreApplication::processEvents(QEventLoop::AllEvents, 1);
            return false;
        }
...
//        qDebug()<<__PRETTY_FUNCTION__<<"tmpFileList = "<<tmpFileList;
        if (tmpFileList.size() > 0)
        {
            ...
            if(!zip.open(QuaZip::mdCreate))
            {
                qDebug()<<__PRETTY_FUNCTION__<<"lines = "<<__LINE__<<" zip.open(QuaZip::mdCreate) open error";
                emit error();
                QCoreApplication::processEvents(QEventLoop::AllEvents, 1);
                return false;
            }
            ...
            for (int i = 0; i < totalf; ++i)
            {
                ...
                if(outFile.getZipError() != UNZ_OK)
                {
                    qDebug()<<__PRETTY_FUNCTION__<<"lines = "<<__LINE__<<" outFile.getZipError() != UNZ_OK";
                    emit message(QString("Zip %1 error!").arg(tmpNameOfZip));
                    QCoreApplication::processEvents(QEventLoop::AllEvents, 1);
                    continue;
                }
...
bool ZipFunction::Unzip(QString pZipFile, QString pDstPath, bool pCover, bool pIngore)
{
    ...
    {
        IDE_TRACE();
        emit error();qDebug()<<__PRETTY_FUNCTION__<<"lines = "<<__LINE__<<" !QFile::exists(pZipFile) errors";
        QCoreApplication::processEvents(QEventLoop::AllEvents, 1);
        return false;
    }
    ...
    {
        IDE_TRACE();
        emit error();qDebug()<<__PRETTY_FUNCTION__<<"lines = "<<__LINE__<<" !zip.open(QuaZip::mdUnzip) errors";
        QCoreApplication::processEvents(QEventLoop::AllEvents, 1);
        return false;
    }

    //>@发送开始信号
    emit workStart("unZip");
    qDebug()<<__PRETTY_FUNCTION__<<"lines = "<<__LINE__<<"workstart";
    QCoreApplication::processEvents(QEventLoop::AllEvents, 1);
...
//            IDE_TRACE_STR(tmpString);
        ...
        {
            qDebug()<<__PRETTY_FUNCTION__<<"lines = "<<__LINE__<<" infile.getZipError() != UNZ_OK";
...
//        emit message(QString("Unzip %1 success!\n").arg(tmpFileName));
        QtSleep(1);
    }
    zip.close();
...
//        emit message(QString("Unzip %1 to %2 error!").arg(pZipFile).arg(pDstPath));
        qDebug()<<__PRETTY_FUNCTION__<<"lines = "<<__LINE__<<" infile.getZipError() != UNZ_OK error";
        emit error();
        QCoreApplication::processEvents(QEventLoop::AllEvents, 1);
        return false;
    }
    emit unZipProgress(100); // send 100
#if (defined(UBUNTU) || defined(LINUX))
    system("sync");
#endif
...
//    IDE_TRACE();
    emit end("unZip");
    qDebug()<<__PRETTY_FUNCTION__<<"lines = "<<__LINE__<<"UnZip is ended!";
    QCoreApplication::processEvents(QEventLoop::AllEvents, 1);
    qDebug()<<__PRETTY_FUNCTION__<<"lines = "<<__LINE__<<"UnZip is ended return true!";
    return true;
}
