...
extern ODCallback_t Extension_output_callbacks[25]; //RPDO1's Callback
...
{
    if(canOpenCallbackClient)
        canOpenCallbackClient->sSDOArrived(1,bSubindex);
    return 0;
}
...
{
    if(canOpenCallbackClient)
        canOpenCallbackClient->sSDOArrived(2,bSubindex);
    return 0;
}
...
int initCANDevice()
{
    char* LibraryPath="/usr/bst/usrfs/libcanfestival_can_socket.so";
    TimerInit();
    ...
    registerCallbacks();
    CAN_PORT  tmpID;  //
    tmpID = canOpen(&CopSlaveBoard,&display_Data); //
    if(!tmpID)  //
    {
        IDE_TRACE_STR("false");
...
//    tmpMSg.data[6]= 0x00;
//    tmpMSg.data[7]= 0x00;
//    tmpint =(int)canSend(tmpID, &tmpMSg); //
//    IDE_TRACE_INT(tmpint);
...
{
    Sound_callbacks[4] = RPDO1_Callback;
    ...
    Display_parameters_callbacks[1] = SDO_2000_Callback;
    Display_parameters_callbacks[2] = SDO_2000_Callback;
    Display_parameters_callbacks[3] = SDO_2000_Callback;
    Display_parameters_callbacks[4] = SDO_2000_Callback;
    Customer_Logo_callbacks[1] = SDO_2001_Callback;
    Customer_Logo_callbacks[2] = SDO_2001_Callback;
