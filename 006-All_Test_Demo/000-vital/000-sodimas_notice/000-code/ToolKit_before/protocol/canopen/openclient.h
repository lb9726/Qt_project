...
private:
    ...
    static OpenClient *m_pInstance;
    class Garbo
        {
        public:
            ~Garbo()
            {
                if (NULL != OpenClient::m_pInstance)
                {
//                    qDebug() << "Garbo dtor";
                    delete OpenClient::m_pInstance;
                }
            }
        };
    static Garbo garbo;
...
//    void rPDO3Arrived();
//    void rPDO4Arrived();
//    void linkCharacter(int curLine, int curClume, int isReal);
    void linkCharacterForRpdo3(int curLine, int curClume, int isReal);
    void linkCharacterForRpdo4(int curLine, int curClume, int isReal);
    void myRPDO3Arrived();
    void myRPDO4Arrived();
    void buildMessageAndEmitSignal(bool pflag, int curline, int curcolumn);
    void resetAfterIsEmpty(int curline, int isReal);
...
public slots:
    ...
    void specialButtonPressed(int btIndex);
