...
bool LogicThemeManager::SetAutoTest(bool pFlag)
{
    ...
    if(pFlag)
    {
        m_Elevator->mAutoTestEnable = true;
    ...
    {
        m_Elevator->mAutoTestEnable = false;
...
#endif
#ifdef DISPLAY
        if(!m_Elevator.isNull())
            m_Elevator->Clear();
#endif
        m_UiContainer->OperateUI(OPERATE_CLEAR, tmpList);
...
void LogicThemeManager::slot_Initialize()
{
    ...
    {
#ifdef DISPLAY
        m_Elevator = VAR_CAST<PhysicalCOM*>(m_DevManager->m_PHDevList.value(P_COM));
        if(m_Elevator.isNull())
            m_Elevator = VAR_CAST<PhysicalSAF*>(m_DevManager->m_PHDevList.value(P_SAF));
#endif
    }
